...
namespace RoSatGCS.Utils.Satellites.Propagation
{
	...
	internal class NoradSGP4 : NoradBase
	{
         ...
         base(orbit)
      {
         ...
         m_omgcof = Orbit.Elements.BStar * m_c3 * Math.Cos(Orbit.Elements.ArgPerigeeRad);
         m_xmcof  = -(2.0 / 3.0) * m_coef * Orbit.Elements.BStar * Globals.Ae / m_eeta;
         m_delmo  = Math.Pow(1.0 + m_eta * Math.Cos(Orbit.Elements.MeanAnomalyRad), 3.0);
         m_sinmo  = Math.Sin(Orbit.Elements.MeanAnomalyRad);
      }
      ...
      public override EciCoordinate GetPosition(double tsince)
      {
         ...
         if (!isimp)
         {
            ...
            tempe = tempe + Orbit.Elements.BStar * m_c5 * (Math.Sin(xmp) - m_sinmo);
