...
namespace RoSatGCS.Utils.Satellites.Propagation
{
	/// <summary>
	...
	internal class NoradSDP4 : NoradBase
	{
      ...
      readonly double dp_e3;     readonly double dp_ee2;    readonly double dp_se2;
      readonly double dp_se3;    readonly double dp_sgh2;   readonly double dp_sgh3;   readonly double dp_sgh4;
      readonly double dp_sh2;    readonly double dp_sh3;    readonly double dp_si2;
      readonly double dp_si3;    readonly double dp_sl2;    readonly double dp_sl3;    readonly double dp_sl4;
      readonly double dp_xgh2;   readonly double dp_xgh3;   readonly double dp_xgh4;   readonly double dp_xh2;
      readonly double dp_xh3;    readonly double dp_xi2;    readonly double dp_xi3;    readonly double dp_xl2;
      readonly double dp_xl3;    readonly double dp_xl4;    readonly double dp_xqncl;  readonly double dp_zmol;
      readonly double dp_zmos;

               double dp_atime;  readonly double dp_d2201;  readonly double dp_d2211;  readonly double dp_d3210;
      readonly double dp_d3222;  readonly double dp_d4410;  readonly double dp_d4422;  readonly double dp_d5220;
      readonly double dp_d5232;  readonly double dp_d5421;  readonly double dp_d5433;  readonly double dp_del1;
      readonly double dp_del2;   readonly double dp_del3;   readonly double dp_omegaq; readonly double dp_sse;
      readonly double dp_ssg;    readonly double dp_ssh;    readonly double dp_ssi;    readonly double dp_ssl;    
      readonly double dp_step2;  readonly double dp_stepn;  readonly double dp_stepp;  readonly double dp_thgr;
      readonly double dp_xfact;  readonly double dp_xlamo;           double dp_xli;             double dp_xni;

      readonly bool gp_reso;  // geopotential resonant
      readonly bool gp_sync;  // geopotential synchronous

      // ///////////////////////////////////////////////////////////////////////////
      public NoradSDP4(Orbit orbit) :
         base(orbit)
      {
         double sinarg = Math.Sin(Orbit.Elements.ArgPerigeeRad);
         double cosarg = Math.Cos(Orbit.Elements.ArgPerigeeRad);
         ...
         double eq = Orbit.Elements.Eccentricity;
         double aqnv = 1.0 / Orbit.SemiMajorRec;

         dp_xqncl = Orbit.Elements.InclinationRad;

         double xmao = Orbit.Elements.MeanAnomalyRad;
         double xpidot = m_omgdot + m_xnodot;
         double sinq = Math.Sin(Orbit.Elements.RAANodeRad);
         double cosq = Math.Cos(Orbit.Elements.RAANodeRad);

         dp_omegaq = Orbit.Elements.ArgPerigeeRad;
         ...
         double zn = zns;
         double ze = zes;
         double xnoi = 1.0 / Orbit.MeanMotionRec;
         ...
         double eosq = Globals.Sqr(Orbit.Elements.Eccentricity);
         ...
         for (int pass = 1; pass <= 2; pass++)
         {
            ...
            if (Orbit.Elements.InclinationRad < 5.2359877E-2)
            {
         ...
         if ((Orbit.MeanMotionRec > 0.0034906585) && (Orbit.MeanMotionRec < 0.0052359877))
         {
            ...
            dp_xlamo = xmao + Orbit.Elements.RAANodeRad + Orbit.Elements.ArgPerigeeRad - dp_thgr;
            bfact = m_xmdot + xpidot - thdt;
            bfact = bfact + dp_ssl + dp_ssg + dp_ssh;

            #endregion
         }
         else if (((Orbit.MeanMotionRec >= 8.26E-3) && (Orbit.MeanMotionRec <= 9.24E-3)) && (eq >= 0.5))
         {
            ...
            dp_xlamo = xmao + Orbit.Elements.RAANodeRad + Orbit.Elements.RAANodeRad - dp_thgr - dp_thgr;
                               ...
                               ref double tsince)
      {
         ...
         emm    = Orbit.Elements.Eccentricity    + dp_sse * tsince;
         xincc  = Orbit.Elements.InclinationRad  + dp_ssi * tsince;
         ...
         if (gp_reso) 
         {
            while (!fDone)
            {
               if ((dp_atime == 0.0)                     ||
                  ...
                  ((tsince <  0.0) && (dp_atime >= 0.0)))
               {
                  delta =  (tsince < 0) ? dp_stepn : dp_stepp;
               ...
               else
               {
                  if (Math.Abs(tsince) < Math.Abs(dp_atime))
                  {
                     ...
                     DeepCalcIntegrator(ref xndot, ref xnddt, ref xldot, delta);
                  }
                  else
                  {
                     delta = dp_stepn;
      ...
      public override EciCoordinate GetPosition(double tsince)
      {
         ...
         double omgadf = Orbit.Elements.ArgPerigeeRad  + m_omgdot * tsince;
         double xnoddf = Orbit.Elements.RAANodeRad + m_xnodot * tsince;
         double tsq    = tsince * tsince;
         double xnode  = xnoddf + m_xnodcf * tsq;
         double tempa  = 1.0 - m_c1 * tsince;
         double tempe  = Orbit.Elements.BStar * m_c4 * tsince;
